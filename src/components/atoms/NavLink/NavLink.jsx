...
const NavLink = ({ href, children, isActive, dropdownItems = [] }) => {
  ...
  return (
    ...
    >
      <a
        href={href}
        className={`px-4 py-2 font-medium transition-all duration-300 hover:text-orange-500
        flex items-center justify-between lg:justify-start gap-1 border-b lg:border-none
        ${isActive ? 'text-orange-500' : 'text-gray-700'}`}
      >
        {children}
        {hasDropdown && <FaChevronDown className={`text-xs transition-transform duration-300 
        ${showDropdown ? 'rotate-180' : ''}`} />}
      </a>

      {hasDropdown && showDropdown && (
        <div className={`lg:absolute lg:top-full lg:left-0 w-full lg:w-60 bg-white 
